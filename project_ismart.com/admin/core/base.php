...
function get_module() {
    global $config;
    $module = isset($_GET['mod']) ? $_GET['mod'] : $config['default_module']; //  mặc định là home
    return $module;
}
...
function get_controller() {
    global $config;
    $controller = isset($_GET['controller']) ? $_GET['controller'] : $config['default_controller']; // mặc định là index
    return $controller;
}
...
function get_action() {
    global $config;
    $action = isset($_GET['action']) ? $_GET['action'] : $config['default_action'];  // mặc định là index
    return $action;
}
...
function load_view($name, $data_send = array()) {
    ...
    /* Ví dụ bên controller gửi data send :     
    * $data['list_users'] = $list_users;
    * load_view('index',$data);
    * => bên view nhận dữ liệu: $key_data = "list_users". 
    * Vậy $$key_data = $list_users
    */
}
